...
export function PriceList({ items }: { items: PriceItem[] }) {
  ...
  return (
    <ul className="divide-y divide-line border-y border-line">
      {active.map((p) => (
        <li key={p.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-baseline gap-6 py-6 md:grid-cols-[minmax(0,3fr)_minmax(0,2fr)_auto]">
          ...
          <div className="text-right">
            <div className="text-display text-2xl text-ink">
              {p.price === 0 ? "Besplatno" : fmt(p.price)}
            </div>
            {p.price > 0 && (
