...
export function DocumentList({
  ...
  const filtered = useMemo(() => {
    return documents
      .filter((d) => (year === "all" ? true : d.year === Number(year)))
    ...
    <div>
      <div className="flex flex-col gap-4 border-y border-line py-6 md:flex-row md:items-center md:justify-between">
        <div className="relative flex items-center gap-3">
          <Search size={16} strokeWidth={1.5} className="text-ink-muted" />
          <input
        ...
        <div className="flex items-center gap-2">
          <label className="text-eyebrow text-ink-muted" htmlFor="year">Godina</label>
          <select
            id="year"
            value={year}
      ...
      <ul className="divide-y divide-line">
        {filtered.map((d) => {
          const cat = DocumentRepo.category(d.category);
          return (
            <li key={d.id} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-6 py-6">
              <div className="grid h-12 w-12 shrink-0 place-items-center border border-line font-mono text-[10px] text-ink-muted">
                {d.fileType}
              </div>
              <div className="min-w-0">
                <p className="text-base text-ink">{d.title}</p>
                <p className="mt-1 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[11px] uppercase tracking-widest text-ink-muted">
                  <span>{new Date(d.publishedAt).toLocaleDateString("hr-HR")}</span>
                  {showCategory && cat && <span>{cat.name}</span>}
        ...
        {filtered.length === 0 && (
          <li className="flex flex-col items-center gap-3 py-16 text-center text-ink-muted">
            ...
            <p className="text-sm">Nema dokumenata za odabrane filtere.</p>
          </li>
        )}
      </ul>
    </div>
